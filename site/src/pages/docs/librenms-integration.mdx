...
These are things that can be added to the project when you're ready (nothing is changed yet):

- `nfs-server-setup.sh` — script for the VPS: installs `nfs-kernel-server`, writes `/etc/exports` from your answers, `exportfs`
- `nfs-client-setup.sh` — script for the LibreNMS server: installs `nfs-common`, mounts, writes `/etc/fstab`
- `librenms-nfsen-config.sh` — runs all the `lnms config:set` commands above
- A LibreNMS docker-compose example that mounts the NFS share as volumes
